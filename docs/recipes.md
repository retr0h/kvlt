...
matching one of the public keys in `vaults/local_encryption/<id>.yaml`.
...
git add vaults/ .kvlt/        # commit recipient list + encrypted blobs
```
...
You can run multiple vaults; the names are local to each repository's `vaults/`
directory. Common patterns:
